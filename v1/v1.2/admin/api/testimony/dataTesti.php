...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    while($row = $stmt->fetch()){
        ...
        ';
        if($row['status'] == 2){
            $row1['action'] .= '<button class="btn btn-light pubButtonTesti" thebool=0 target="'.$row['testi_id'].'">Unpublish</button></div>';
        }else{
            $row1['action'] .= '<button class="btn btn-warning pubButtonTesti" thebool=1 target="'.$row['testi_id'].'" >Publish</button></div>';
        }
