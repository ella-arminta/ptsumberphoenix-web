...
if ($_SERVER['REQUEST_METHOD'] == 'POST'){
    $bool = $_POST['bool'];
    $code = $_POST['code'];
    $stmt = $conn->prepare("UPDATE products set featured = ? where product_code = ? and status = 1");
    ...
    if($berhasil){
        ...
        $proName = $proName['product_name'];
         ...
         if($bool == 1){
            $desc = $namaAdm.' has set feature for product named '.$proName;
            $action = 'set featured';
         }else{
            $desc = $namaAdm.' has remove feature for product named '.$proName;
            $action = 'remove featured';
         }
