...
(function() {
				...
				on: function(b, d, h, k, g) {
					...
					if (0 > e.getListenerIndex(d)) {
						...
						for (var y = e.length - 1; 0 <= y; y--)
							if (e[y].priority <= g)
								return e.splice(y + 1, 0, m), { removeListener: r };
						e.unshift(m);
					}
						...
						if (e) {
							...
							if (y.length)
								for (var y = y.slice(0), A, D = 0; D < y.length; D++) {
									if (e.errorProof)
										try {
											A = y[D].call(this, r, m, b, k);
										} catch (x) {}
									else A = y[D].call(this, r, m, b, k);
		...
		CKEDITOR.tools = {
			...
			extend: function(a) {
				...
				"boolean" == typeof (b = arguments[e - 1])
					? e--
					: "boolean" == typeof (b = arguments[e - 2]) &&
					  ((c = arguments[e - 1]), (e -= 2));
				for (var g = 1; g < e; g++) {
					var d = arguments[g],
						m;
			...
			convertRgbToHex: function(a) {
				return a.replace(/(?:rgb\(\s*(\d+)\s*,\s*(\d+)\s*,\s*(\d+)\s*\))/gi, function(
					a,
					b,
					c,
					g
			...
			keystrokeToArray: function(a, e) {
				...
				c &&
					(a[c]
						? (d.push(a[c]), m.push(a[c]))
						: (d.push(String.fromCharCode(c)), m.push(String.fromCharCode(c))));
			...
			getCookie: function(a) {
				...
				for (var e = document.cookie.split(";"), b, c, g = 0; g < e.length; g++)
					if (
						((b = e[g].split("\x3d")),
						(c = decodeURIComponent(CKEDITOR.tools.trim(b[0]).toLowerCase())),
						c === a)
					)
			...
			style: {
				parse: {
					...
					margin: function(a) {
						...
						var c = {},
							g = a.match(/(?:\-?[\.\d]+(?:%|\w*)|auto|inherit|initial|unset)/g) || [
								"0px"
							];
						switch (g.length) {
							case 1:
			...
			object: {
				...
				merge: function(a, b) {
					...
					c.array.forEach(c.objectKeys(d), function(a) {
						g[a] =
							"object" === typeof d[a] && "object" === typeof g[a]
								? c.object.merge(g[a], d[a])
								: d[a];
					});
					return g;
				}
		...
		CKEDITOR.tools.extend(CKEDITOR.dom.element.prototype, {
			...
			removeStyle: function(a) {
				...
				if (b.removeProperty || ("border" != a && "margin" != a && "padding" != a))
					...
					for (var b = [], d = 0; d < c.length; d++)
						if (e) for (var y = 0; y < e.length; y++) b.push([a, c[d], e[y]].join("-"));
				...
				if (
					...
					k &&
						(CKEDITOR.env.webkit || (CKEDITOR.env.ie && 12 <= CKEDITOR.env.version)
							...
							: ((c = y ? d.$ : h), (b = c.scrollLeft), (c = c.scrollTop)),
						(b = f.left + b - p),
						(c = f.top + c - x));
				} else
					for (
			...
			scrollIntoParent: function(a, b, c) {
				...
				function x(a, b) {
					...
					e = a.getWindow();
					e.equals(b) ||
						((e = x(CKEDITOR.dom.element.get(e.$.frameElement), b)),
						(c.x += e.x),
						(c.y += e.y));
					return c;
				}
			...
			copyAttributes: function(a, b) {
				...
				for (var e = 0; e < c.length; e++) {
					...
					if (!(y in b))
						...
						else if (!CKEDITOR.env.ie || this.hasAttribute(y))
	...
	(function() {
						...
						? e.type == CKEDITOR.NODE_ELEMENT &&
						  (e =
								0 < z
									...
									: !1 === h(e, !0)
									? null
									: e.getPreviousSourceNode(!0, q, h))
						: ((e = d),
						  e.type == CKEDITOR.NODE_ELEMENT &&
								((e = e.getChild(f)) ||
									(e = !1 === h(d, !0) ? null : d.getNextSourceNode(!0, q, h)))),
				  e && !1 === h(e) && (e = null));
			for (; e && !this._.end; ) {
				this.current = e;
		...
		var e = CKEDITOR.dom.walker.empty(),
			z = (CKEDITOR.dom.walker.validEmptyBlockContainers = CKEDITOR.tools.extend(
				(function(a) {
					var b = {},
						c;
					...
					: b.type == CKEDITOR.NODE_TEXT ||
					  (b.type == CKEDITOR.NODE_ELEMENT &&
							(b.is(CKEDITOR.dtd.$inline) ||
								...
								(!CKEDITOR.env.needsBrFiller && b.is(z) && e(b))))
					? !0
					: !1;
				return !!(a ^ b);
			};
		...
		CKEDITOR.dom.element.prototype.getBogus = function() {
			...
			while (m(a));
			return a &&
				(CKEDITOR.env.needsBrFiller ? a.is && a.is("br") : a.getText && f.test(a.getText()))
				? a
				: !1;
		};
	})();
	...
	(function() {
		...
		function d(a, b, c, d, f) {
			...
			if (
				...
				c.append(a.document.createText(F.substring(L, C)));
			else {
				F.type == CKEDITOR.NODE_TEXT
					? l
						? (P = !0)
				...
				G = c;
				for (Q = N; Q <= B; Q++)
					if (((c = n[Q]), (V = c.getPrevious()), c.equals(O[Q])))
						b && (G = G.getChild(0));
					else {
						Q != B || (c.equals(O[Q]) && B < v)
							? b && (I = G.append(c.clone(0, f)))
							: t
							? h(c, G, !1, w)
							: P && G.append(a.document.createText(c.substring(0, C)));
		...
		CKEDITOR.dom.range.prototype = {
				...
				function a(b) {
					...
					p =
						p.type != CKEDITOR.NODE_ELEMENT || 0 === f || f == p.getChildCount()
							? 0
							: p.getChild(f - 1).type == CKEDITOR.NODE_TEXT &&
							  p.getChild(f).type == CKEDITOR.NODE_TEXT;
			...
			enlarge: function(a, b) {
				...
				switch (a) {
					...
					case CKEDITOR.ENLARGE_ELEMENT:
						...
						C.type == CKEDITOR.NODE_TEXT
							? (t &&
									((C = !CKEDITOR.tools.trim(C.substring(0, t)).length && C),
									(L = !!C)),
							  C && ((q = C.getPrevious()) || (l = C.getParent())))
							: (t && (q = C.getChild(t - 1) || C.getLast()), q || (l = C));
						for (l = c(l); l || q; ) {
							if (l && !q) {
								!F && l.equals(p) && (F = !0);
								if (f ? l.isBlockBoundary() : !g.contains(l)) break;
								(L && "inline" == l.getComputedStyle("display")) ||
									((L = !1), F ? (k = l) : this.setStartBefore(l));
								q = l.getPrevious();
							}
							for (; q; )
								if (((C = !1), q.type == CKEDITOR.NODE_COMMENT))
									q = q.getPrevious();
								else {
									if (q.type == CKEDITOR.NODE_TEXT)
										...
										if (L && CKEDITOR.dtd.$removeEmpty[q.getName()]) {
											...
											if (d.test(M)) q = null;
												...
												for (
													var t = q.$.getElementsByTagName("*"), E = 0, w;
													(w = t[E++]);
						...
						F = L = !1;
						C.type == CKEDITOR.NODE_TEXT
							? CKEDITOR.tools.trim(C.substring(t)).length
								? (L = !0)
								: ((L = !C.getLength()),
								  t == C.getLength()
										? (q = C.getNext()) || (l = C.getParent())
										: h(C, t) && (l = C.getParent()))
							: (q = C.getChild(t)) || (l = C);
						for (; l || q; ) {
							if (l && !q) {
								...
								if (f ? l.isBlockBoundary() : !g.contains(l)) break;
								(L && "inline" == l.getComputedStyle("display")) ||
									((L = !1), F ? (r = l) : l && this.setEndAfter(l));
								q = l.getNext();
							}
							for (; q; ) {
								C = !1;
					...
					case CKEDITOR.ENLARGE_LIST_ITEM_CONTENTS:
						...
						this.setStartAt(
							H,
							!H.is("br") &&
								((!l && this.checkStartOfBlock()) || (l && H.contains(l)))
			...
			moveToClosestEditablePosition: function(a, b) {
				...
				a
					? ((c = new CKEDITOR.dom.range(this.root)), c.moveToPosition(a, p[b ? 0 : 1]))
					: (c = this.clone());
	...
	(function() {
		...
		function b(a, c, f, h) {
			...
			if (!h) return 0;
			if ((g = CKEDITOR.filter.instances[h.element.data("cke-filter")]) && !g.check(c))
		...
		a.prototype = {
			getNextParagraph: function(a) {
				...
				if (!this._.started) {
					...
					x.collapsed ||
						((k = new CKEDITOR.dom.walker(x.clone())),
						...
						(k.evaluator = p),
						(k = k.previous()),
						(this._.lastNode = k.getNextSourceNode(!0, null, x.root)),
						this._.lastNode &&
							...
							p.checkEndOfBlock() &&
								...
								(this._.lastNode = (p.block || p.blockLimit).getNextSourceNode(
									!0
								)))),
				...
				for (this._.nextNode = null; p; ) {
					...
					if (K) p.type == CKEDITOR.NODE_TEXT && f.test(p.getText()) && (K = 0);
						...
						if (
							...
							k &&
								(k.setEndAt(p, CKEDITOR.POSITION_BEFORE_START),
								"br" != q && (this._.nextNode = p));
							J = 1;
						} else {
							if (p.getFirst()) {
								k ||
					...
					K &&
						!k &&
						((k = this.range.clone()), k.setStartAt(p, CKEDITOR.POSITION_BEFORE_START));
					y = (!J || K) && p.equals(x);
					if (k && !J)
						for (; !p.getNext(g) && !y; ) {
							q = p.getParent();
					...
					!p.equals(k.root)
						...
						: y ||
						  (this._.nextNode = d.equals(x)
								? null
								: this._getNextSourceNode(k.getBoundaryNodes().endNode, 1, x));
				}
				A &&
					(A = d.getPrevious()) &&
					A.type == CKEDITOR.NODE_ELEMENT &&
				...
				D &&
					...
					A.type == CKEDITOR.NODE_ELEMENT &&
					"br" == A.getName() &&
					(!CKEDITOR.env.needsBrFiller || A.getPrevious(h) || A.getNext(h)) &&
					A.remove();
				this._.nextNode ||
					(this._.nextNode =
						y || d.equals(x) || !x ? null : this._getNextSourceNode(d, 1, x));
				return d;
			},
	...
	(function() {
		function a(a, b, c, d, f) {
			...
			for (e in b) {
				...
				for (p in P) {
					...
					t ? (g = !1) : (t = {});
					for (H in w)
						"!" == H.charAt(0) && ((H = H.slice(1)), B.push(H), (t[H] = !0), (g = !1));
					for (; (H = B.pop()); ) (w[H] = w["!" + H]), delete w["!" + H];
					h[q] = (g ? !1 : t) || null;
				}
					...
					if (
						...
						if (a)
							if (!0 === a) a = !0;
								...
								for (var e = 0, v = c.length, l; e < v; ++e)
									(l = c[e]), f[l] || (f[l] = a(l));
								a = !1;
							}
						else a = !1;
		...
		function k(a, b) {
			...
			if (!0 === a) return a;
			if ("string" == typeof a)
				return (a = w(a)), "*" == a ? !0 : CKEDITOR.tools.convertArrayToObject(a.split(b));
			if (CKEDITOR.tools.isArray(a))
		...
		function e(a) {
			...
			for (a = w(a); (b = a.match(n)); )
				(c = b[2])
					? ((d = z(c, "styles")), (f = z(c, "attrs")), (c = z(c, "classes")))
					: (d = f = c = null),
			...
			if (e.doFilter) {
				a: {
					...
					if (a || n) {
						...
						if (t) for (k = 0, g = t.length; k < g; ++k) c(t[k], b, E);
						if (a) for (k = 0, g = a.length; k < g; ++k) d(a[k], b, E, w);
						if (n) for (k = 0, g = n.length; k < g; ++k) d(n[k], b, E, w);
						a = E;
					} else a = null;
				}
			...
			if (t.$empty[d] || !a.children.length)
				...
				if ("br" == b)
					...
					if (f)
						...
						f = a.parent;
						for (
							var e = f.type == CKEDITOR.NODE_DOCUMENT_FRAGMENT || "body" == f.name,
								n,
								h,
								v = d.length;
							0 < v;
							...
							(l = d[--v]),
								...
								(l.type == CKEDITOR.NODE_TEXT ||
									(l.type == CKEDITOR.NODE_ELEMENT && t.$inline[l.name]))
									? (n ||
		...
		CKEDITOR.filter.prototype = {
			...
			applyTo: function(a, b, c, d) {
				...
				for (; (w = a.pop()); )
					if (((c = w.el), c.parent))
						switch (((E = t[c.parent.name] || t.span), w.check)) {
							case "it":
					...
					for (f = 0; f < a.length; ++f) {
						...
						for (v = 0; v < b.length; ++v)
							(l = b[v]),
								...
								h instanceof CKEDITOR.style && (h = C(h)),
								c.push({
									check: n == e ? null : n,
									left: h,
			...
			check: function(a, b, c) {
				...
				if ("string" == typeof a) {
					...
					(d = a.getDefinition()),
						...
						f && !CKEDITOR.tools.isEmpty(f)
							? ((f = E(f)), (l.style = CKEDITOR.tools.writeCssText(f, !0)))
							: (f = {}),
						(f = {
							name: d.element,
							attributes: l,
				...
				b =
					0 < n.length
						? !1
						: CKEDITOR.tools.objectCompare(f.attributes, l.attributes, !0)
						? !0
						: !1;
										...
										(8 >= CKEDITOR.env.version || CKEDITOR.env.ie9Compat)
											? (e.$.onreadystatechange = function() {
													if (
														...
														(e.$.onreadystatechange = null), y(b, !0);
											  })
											: ((e.$.onload = function() {
													setTimeout(function() {
														y(b, !0);
	...
	CKEDITOR.resourceManager.prototype = {
		...
		load: function(a, d, b) {
			...
			for (
				var c = this.loaded, f = this.registered, h = [], k = {}, g = {}, m = 0;
				m < a.length;
				m++
			) {
				var r = a[m];
					...
					a.call(
						...
						function(a) {
							...
							for (e in a) {
								...
								if (!d[e]) {
									if (g.icons)
										for (var A = g.icons.split(","), D = A.length; D--; )
											CKEDITOR.skin.addIcon(
												A[D],
												g.path +
													"icons/" +
													(CKEDITOR.env.hidpi && g.hidpi
														? "hidpi/"
														: "") +
													A[D] +
													".png"
								...
								if (y)
									for (y.split && (y = y.split(",")), g = 0; g < y.length; g++)
										h[y[g]] || b.push(y[g]);
							}
			...
			CKEDITOR.plugins.load(c.split(","), function(c) {
				...
				for (var l in c) {
					...
					q &&
						!a.lang[l] &&
						(q.split && (q = q.split(",")),
						0 <= CKEDITOR.tools.indexOf(q, a.langCode)
							? (k = a.langCode)
							: ((k = a.langCode.replace(/-.*/, "")),
							  (k =
									k != a.langCode && 0 <= CKEDITOR.tools.indexOf(q, k)
										? k
										: 0 <= CKEDITOR.tools.indexOf(q, "en")
										? "en"
										: q[0])),
						w.langEntries && w.langEntries[k]
							? ((a.lang[l] = w.langEntries[k]), (k = null))
							: e.push(CKEDITOR.getUrl(w.path + "lang/" + k + ".js")));
					f.push(k);
					d.push(w);
				}
					...
					for (var c = ["beforeInit", "init", "afterInit"], e = 0; e < c.length; e++)
						for (var l = 0; l < d.length; l++) {
							...
							0 === e &&
								...
								n.lang &&
								n.langEntries &&
								(a.lang[n.name] = n.langEntries[f[l]]);
							if (n[c[e]]) n[c[e]](a);
						}
			...
			for (var f = new CKEDITOR.dom.documentFragment(), e, h, k, g = 0; g < a.length; g++) {
				...
				c(M)
					? (e ||
					  ...
					  (h && h.equals(t)) || ((h = t), (k = t.clone()), e.append(k)),
					  k.append(d(M)))
			...
			parse: function(b) {
				for (var c, f, h = 0, k; (c = this._.htmlPartsRegex.exec(b)); ) {
					...
					if ((f = c[1]))
						if (
							((f = f.toLowerCase()),
							k && CKEDITOR.dtd.$cdata[f] && (this.onCDATA(k.join("")), (k = null)),
							!k)
						) {
							this.onTagClose(f);
							continue;
	...
	(function() {
		...
		CKEDITOR.htmlParser.fragment.fromHtml = function(h, k, g) {
			...
			function z(b, c, d) {
				...
				var f = u;
				void 0 === b.previous &&
					(y(c, b) && ((u = c), D.onTagOpen(g, {}), (b.returnPoint = c = u)),
					e(b),
				...
				if (a(e)) p.push(e);
					...
					if ("br" == f) J.push(e);
						...
						for (
							...
							!((k = (h = u.name)
								? CKEDITOR.dtd[h] ||
								  (u._.isBlockLike ? CKEDITOR.dtd.div : CKEDITOR.dtd.span)
								: c),
							e.isUnknown || u.isUnknown || k[f]);
							...
							if (u.isOptionalClose) D.onTagClose(h);
								...
								(h = u.children),
									((h = h[h.length - 1]) && "li" == h.name) ||
										z((h = new CKEDITOR.htmlParser.element("li")), u),
									!e.returnPoint && (e.returnPoint = u),
									(u = h);
							else if (f in CKEDITOR.dtd.$listItem && !A(f, h))
								D.onTagOpen("li" == f ? "ul" : "dl", {}, 0, 1);
							else if (h in d && !A(f, h))
								!e.returnPoint && (e.returnPoint = u), (u = u.parent);
							else if ((h in CKEDITOR.dtd.$inline && p.unshift(u), u.parent))
								z(u, u.parent, 1);
							else {
								e.isOrphan = 1;
								break;
			...
			D.onText = function(a) {
				if (
					!((u._.hasInlineStarted && !J.length) || l || K) &&
	...
	(function() {
			...
			function t(a, b) {
				...
				return a.type == CKEDITOR.NODE_TEXT &&
					...
					(c.index &&
						...
						(a.value = c[0])),
					(!CKEDITOR.env.needsBrFiller && E && (!b || a.parent.name in g)) ||
						(!E && (((c = a.previous) && "br" == c.name) || !c || h(c))))
					? !0
					: !1;
			}
			var w = { elements: {} },
		...
		function x(a, b) {
			...
			return (a = a.replace(/<(title|iframe|textarea)([^>]*)>([\s\S]*?)<\/\1>/g, function(
				a,
				c,
				d,
				f
			...
			b.on(
				...
				function(a) {
					...
					f &&
						((l = c),
						!l.children.length &&
							CKEDITOR.dtd[l.name][f] &&
							((f = new CKEDITOR.htmlParser.element(f)), l.add(f)));
					a.dataValue = c;
				},
				null,
				null,
	...
	(function() {
			...
			proto: {
				...
				insertElementIntoRange: function(a, b) {
					...
					if (f)
						for (
							;
							(t = b.getCommonAncestor(0, 1)) &&
							(E = CKEDITOR.dtd[t.getName()]) &&
							(!E || !E[e]);
				...
				extractHtmlFromRange: function(a, b) {
					...
					if (b) {
						...
						c.autoParagraph(this.editor, a),
							y(a.startContainer) && a.startContainer.appendBogus();
					...
					if (!this.is("textarea")) {
							...
							if (0 !== b.getRanges().length) {
								if (c in t) {
									...
									(f = h(b))
										...
										: g.collapsed &&
										  ((k = n.block) &&
										  (B = k[c ? "getPrevious" : "getNext"](e)) &&
										  B.type == CKEDITOR.NODE_ELEMENT &&
										  B.is("table") &&
										  g[c ? "checkStartOfBlock" : "checkEndOfBlock"]()
												? (a.fire("saveSnapshot"),
												  g[
														c ? "checkEndOfBlock" : "checkStartOfBlock"
												  ]() && k.remove(),
												...
												: n.blockLimit &&
												  ...
												  g.checkBoundaryOfElement(
														v,
														c ? CKEDITOR.START : CKEDITOR.END
												  ) &&
												  (B = v[c ? "getPrevious" : "getNext"](e))
												? (a.fire("saveSnapshot"),
												  g["moveToElementEdit" + (c ? "End" : "Start")](B),
						...
						CKEDITOR.env.gecko &&
							this.attachListener(this, "mouseup", function(b) {
								if (
									2 == b.data.$.button &&
									((b = b.data.getTarget()), !b.getOuterHtml().replace(D, ""))
								) {
									var c = a.createRange();
		...
		x = (function() {
			...
			function b(c, d) {
				...
				-1 < m && (g[m].firstNotAllowed = 1);
				-1 < r && (g[r].lastNotAllowed = 1);
				return g;
			}
			function d(b, c) {
				var e = [],
				...
				for (w && e.push(" "); g < t; g++)
					(h = f.getItem(g)), a(h) && !h.is(k) ? (e = e.concat(d(h, c))) : e.push(h);
				w && e.push(" ");
				return e;
			}
			function e(b) {
			...
			var E = CKEDITOR.dtd,
				...
				p = CKEDITOR.tools.extend({}, E.$inline);
			delete p.br;
			return function(w, n, N, v) {
				...
				if (!v.checkReadOnly()) {
					var I =
						new CKEDITOR.dom.elementPath(v.startContainer, v.root).blockLimit || v.root;
					w = {
						type: n,
						dontFilter: G,
						editable: w,
					...
					m = n.startPath();
					if ((B = m.contains(f, !1, 1)))
						n.splitElement(B),
							(w.inlineStylesRoot = B),
							(w.inlineStylesPeak = m.lastElement);
					m = n.createBookmark();
					(B = m.startNode.getPrevious(c)) && a(B) && f(B) && v.push(B);
					(B = m.startNode.getNext(c)) && a(B) && f(B) && v.push(B);
					for (B = m.startNode; (B = B.getParent()) && f(B); ) v.push(B);
					n.moveToBookmark(m);
					if ((m = N)) {
						m = w.range;
						if ("text" == w.type && w.inlineStylesRoot) {
							...
							for (v = w.inlineStylesRoot.getParent(); !r.equals(v); )
								(n = n.appendTo(r.clone())), (r = r.getParent());
							N = n
								.getOuterHtml()
								.split("{cke-peak}")
								.join(N);
						}
						r = w.blockLimit.getName();
						...
						if ((x = m.startPath().block) && (1 != x.getChildCount() || !x.getBogus()))
							a: {
								var z;
								if (
									1 == r.getChildCount() &&
					...
					if (m) {
						...
						for (t(n, z); v < n.length; v++) {
							...
							if ((u = I.isLineBreak)) {
								...
								I.hasBlockSibling
									? (u = 1)
									: (S = u.startContainer.getAscendant(E.$block, 1)) &&
									  S.is({ div: 1, p: 1 })
									? ((Y = S.getPosition(J)),
									  ...
									  Y == CKEDITOR.POSITION_CONTAINS
											? (u = 0)
											: ((J = u.splitElement(S)),
											  u.moveToPosition(J, CKEDITOR.POSITION_AFTER_START),
											  (u = 1)))
									: (u = 0);
							}
							if (u) N = 0 < v;
								...
								u = z.startPath();
								!I.isBlock &&
									k(w.editor, u.block, u.blockLimit) &&
									(T = g(w.editor)) &&
									((T = m.createElement(T)),
									T.appendBogus(),
									z.insertNode(T),
									CKEDITOR.env.needsBrFiller && (A = T.getBogus()) && A.remove(),
									z.moveToPosition(T, CKEDITOR.POSITION_BEFORE_END));
								...
								I.lastNotAllowed &&
									v < n.length - 1 &&
									((K = y ? G : K) &&
										z.setEndAt(K, CKEDITOR.POSITION_AFTER_START),
									(B = 0));
								z.collapse();
							}
						}
						1 != n.length
							...
							: ((A = n[0]),
							  (A =
									A.isElement &&
									"false" == A.node.getAttribute("contenteditable")));
						A &&
							((N = !0),
							(u = n[0].node),
							z.setStartAt(u, CKEDITOR.POSITION_BEFORE_START),
							z.setEndAt(u, CKEDITOR.POSITION_AFTER_END));
						w.dontMoveCaret = N;
					...
					for (W = A.createBookmark(); (D = w.zombies.pop()); )
						D.getParent() &&
							((T = A.clone()),
							T.moveToElementEditStart(D),
							T.removeEmptyBlocksAtEnd());
					if (K)
						for (; (D = K.pop()); )
							CKEDITOR.env.needsBrFiller
								? D.appendBogus()
								: D.append(A.document.createText(" "));
					for (; (D = w.mergeCandidates.pop()); ) D.mergeSiblings();
					A.moveToBookmark(W);
					if (!w.dontMoveCaret) {
						for (D = e(A); D && a(D) && !D.is(E.$empty); ) {
							if (D.isBlockBoundary())
			...
			return function(d) {
				...
				f.is("table")
					...
					: (f.is({ tbody: 1, thead: 1, tfoot: 1 }) && (f = b(f, "tr", t)),
					  ...
					  d.moveToPosition(
							f,
							t ? CKEDITOR.POSITION_AFTER_START : CKEDITOR.POSITION_BEFORE_END
					  ));
			};
		})();
		J = (function() {
					...
					merge: function(a, c) {
						if (a.mergeListBookmark) {
							...
							a.mergeListItems &&
								...
								(w = w.contains(CKEDITOR.dtd.$listItem)),
								f.equals(w) || b(w, d, e));
							d.remove();
							e.remove();
						}
					...
					function a(c) {
						...
						g.guard = function(a, g) {
							...
							if (g && h && a.equals(h))
								...
								if (!B && (B = g))
									if (a.is(d))
										var B = h && h.getAscendant("table", !0),
					...
					return {
						...
						deleteRanges: function(a) {
							for (var b; (b = a.tableContentsRanges.pop()); )
								b.extractContents(),
									y(b.startContainer) && b.startContainer.appendBogus();
	...
	(function() {
		...
		function h() {
			...
			if (
				!b &&
				((b = c || this.getSelection(1)), !b || b.getType() == CKEDITOR.SELECTION_NONE)
			)
				return;
		...
		function m(a) {
			return L(a) || (a.type == CKEDITOR.NODE_ELEMENT && !a.is(CKEDITOR.dtd.$empty))
				? !0
				: !1;
		}
		function r(a) {
			...
			return b(d) ||
				...
				!(
					d ||
					e ||
					(c.type == CKEDITOR.NODE_ELEMENT && c.isBlockBoundary() && c.getBogus())
				)
				? !0
				: !1;
		}
		...
		CKEDITOR.on("instanceCreated", function(a) {
			...
			c.on("contentDom", function() {
					...
					g.attachListener(g, "mousedown", function(a) {
						2 == a.data.$.button &&
							(((a = c.document.getSelection()) &&
								...
								(y = c.window.getScrollPosition()));
					});
					g.attachListener(g, "mouseup", function(a) {
						2 == a.data.$.button &&
							y &&
							((c.document.$.documentElement.scrollLeft = y.x),
							(c.document.$.documentElement.scrollTop = y.y));
						y = null;
					});
				...
				if (E && (CKEDITOR.env.webkit || CKEDITOR.env.gecko)) {
					...
					g.attachListener(CKEDITOR.env.ie ? g : e.getDocumentElement(), "mouseup", k, c);
				CKEDITOR.env.webkit &&
					g.attachListener(
						e,
						"keydown",
					...
					function() {
						...
						c &&
							(c.getCustomData("ready")
								? (z(a), a.editor.fire("selectionCheck"))
					...
					if (this.isLocked) {
						...
						if (n) this.fake(n);
							...
							if (p) {
								...
								if (
									(b.startContainer.type == CKEDITOR.NODE_ELEMENT &&
										b.startContainer.getName() in v) ||
									(b.endContainer.type == CKEDITOR.NODE_ELEMENT &&
										b.endContainer.getName() in v)
								)
								...
								k
									...
									: ((B = v.getNext(m)),
									  ...
									  (B =
											!(B && B.getText && B.getText().match(n)) &&
											(G ||
												!v.hasPrevious() ||
												(v.getPrevious().is && v.getPrevious().is("br")))),
									  (G = b.document.createElement("span")),
									  G.setHtml("\x26#65279;"),
									  G.insertBefore(v),
									  B && b.document.createText("﻿").insertBefore(v));
								b.setStartBefore(v);
								v.remove();
								a
									? (B
								...
								for (I = 0; I < a.length; I++) {
									if (
										...
										(G = a[I + 1]),
										(n = B.clone()),
										n.setStart(B.endContainer, B.endOffset),
									...
									try {
										...
										if (0 <= y.toString().indexOf("NS_ERROR_ILLEGAL_VALUE"))
				...
				for (q.getPosition(y) == CKEDITOR.POSITION_FOLLOWING && (q = 0); q; ) {
					...
					if (q.equals(y)) (q = null), (l = !0);
						...
						var z = q.type == CKEDITOR.NODE_ELEMENT ? q.getName() : null,
							...
							A = z && q.getAttribute("data-nostyle");
						...
						if (x && m && CKEDITOR.dtd.$block[z])
							for (
								var E = q,
									...
									H = C.length,
									K = 0,
									E = H && new CKEDITOR.dom.range(E.getDocument());
								K < H;
								++K
							) {
								var F = C[K],
						...
						if (C)
							if (
								...
								(K = k),
								!F ||
									(!(F.getDtd() || CKEDITOR.dtd.span)[H] && !K) ||
									(C.parentRule && !C.parentRule(F)))
							)
								l = !0;
							else {
								if (
									...
									z == CKEDITOR.NODE_TEXT ||
										x ||
										(z == CKEDITOR.NODE_ELEMENT && !q.getChildCount()))
								) {
									for (
										var z = q, M;
										(l = !z.getNext(w)) &&
										((M = z.getParent()), p[M.getName()]) &&
										d(M, t, h, O);
				...
				for (
					...
					if (this.checkElementRemovable(k)) {
						...
						(a.checkBoundaryOfElement(k, CKEDITOR.END) ||
							...
							: (k.mergeSiblings(),
							  k.is(this.element) ? A.call(this, k) : x(k, l(this)[k.getName()]));
					}
				if (g) {
					e = d;
					for (h = 0; ; h++) {
			...
			do
				if (m.type == CKEDITOR.NODE_ELEMENT) {
					...
					if (
						!this.lastElement &&
						((this.lastElement = m),
						m.is(CKEDITOR.dtd.$object) || "false" == m.getAttribute("contenteditable"))
					)
						continue;
					if (m.equals(f)) break;
					if (
						!k &&
						((r = m.getName()),
						"true" == m.getAttribute("contenteditable")
							? (k = m)
							: !h && d[r] && (h = m),
						a[r])
					) {
						...
						if (g && 1 < a.length) {
							if (!h)
								for (var m = a.length - 1; 0 <= m; m--)
									d.unshift(a[m].createBookmark(!0));
							if (k)
								for (var r = 0; a[h + r + 1]; ) {
									var e = g.document;
									k = 0;
									...
									for (e = e.getById(d[r + 1].startNode); ; ) {
										...
										if (e.equals(m)) k = 1;
											...
											c(m) ||
											(m.type == CKEDITOR.NODE_ELEMENT && m.isBlockBoundary())
										)
											continue;
										break;
									}
							...
							for (g.moveToBookmark(d.shift()); r--; )
								(m = a[++h]),
									...
									g.setEnd(m.endContainer, m.endOffset);
						}
						return g;
					}
				};
...
*/
		...
		var b = (function() {
				return function(b, d) {
					for (var a = b.match(/[^#]./g), e = 0; 3 > e; e++) {
						...
						c = parseInt(a[e], 16);
						c = (
							"0" + (0 > d ? 0 | (c * (1 + d)) : 0 | (c + (255 - c) * d)).toString(16)
						).slice(-2);
						a[f] = c;
					}
					return "#" + a.join("");
	...
	(function() {
		...
		CKEDITOR.dialog = function(a, b) {
			...
			CKEDITOR.event.call(this);
			this.definition = g = CKEDITOR.fire("dialogDefinition", { name: b, definition: g }, a)
				.definition;
			...
			render: function(a, b) {
				...
				d
					? (k = a.getCommandKeystroke(d)) &&
					  (l = CKEDITOR.tools.keystrokeToString(a.lang.common.keyboard, k))
					: (g += "off");
				...
				for (e = 0; e < b.length; ++e)
					(d = b[e]),
						...
						"/" == d
							...
							: d.items &&
							  (c(h, CKEDITOR.tools.clone(d.items)), (h.name = d.name), a.push(h));
				return a;
			}
	...
	(function() {
		...
		function x(a) {
				...
				return {
					...
					exec: function(a, b) {
						function c(b, h) {
							...
							b
								? ((b.method = "paste"),
								  b.dataTransfer || (b.dataTransfer = p.initPasteDataTransfer()),
								  r(a, b, h))
								: e &&
					...
					a.once("instanceReady", function() {
						a._.pasteButtons &&
							CKEDITOR.tools.array.forEach(a._.pasteButtons, function(b) {
								if ((b = a.ui.get(b)))
									CKEDITOR.document.getById(b._.id).on("touchend", function() {
										a._.forcePasteDialog = !0;
									});
							});
					});
			...
			(function() {
				function b(c, d, e, h, f) {
					...
					a.addCommand(d, e);
					a.ui.addButton &&
						a.ui.addButton(c, { label: n, command: d, toolbar: "clipboard," + h });
					a.addMenuItems &&
						a.addMenuItem(d, { label: n, command: d, group: "clipboard", order: f });
				}
				...
				get: function(c) {
					return "plain-text" == c
						...
						? ((c = b.semanticContent) ||
								...
								c.allow({
									$1: { elements: a(), attributes: !0, styles: !1, classes: !1 }
								}),
		...
		CKEDITOR.plugins.clipboard = {
			...
			getRangeAtDropPosition: function(a, b) {
				...
				if (document.caretRangeFromPoint && b.document.$.caretRangeFromPoint(d, g))
					...
					if (CKEDITOR.env.ie && 8 < CKEDITOR.env.version && h && b.editable().hasFocus)
						return h;
		...
		CKEDITOR.plugins.clipboard.fallbackDataTransfer.prototype = {
			...
			_getFallbackTypeContent: function() {
				...
				a ||
					(a = this._extractDataComment(this._getData(this._customDataFallbackType, !0))
						.content);
				return a;
			},
			_getFallbackTypeData: function() {
				var a = CKEDITOR.plugins.clipboard.fallbackDataTransfer._customTypes,
					b =
						this._extractDataComment(this._getData(this._customDataFallbackType, !0))
							.data || {},
	...
	(function() {
		...
		CKEDITOR.plugins.link = {
			...
			parseLinkAttributes: function(c, b) {
				...
				if (!g.type)
					if ((f = a.match(x)))
						(g.type = "anchor"), (g.anchor = {}), (g.anchor.name = g.anchor.id = f[1]);
					else if ((f = a.match(u))) {
						e = a.match(v);
						a = a.match(w);
						g.type = "email";
